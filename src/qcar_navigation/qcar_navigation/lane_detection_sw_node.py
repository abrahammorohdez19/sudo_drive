...
class LaneDetectionNode(Node):
    ...
    def __init__(self):
        ...
        self.sub = self.create_subscription(
            Image, self.in_topic, self.cb, qos_profile_sensor_data)
    ...
    def cb(self, msg):
        ...
        if self.publish_debug:
            dbg = self._draw(frame, binary, roi_mask, win_boxes,
                             line_pts, poly, cx, cy, base_x, h, w)
    ...
    def _binarize(self, frame):
        ...
        # Morphology: cierra huecos en la cinta
        k = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE,  k)
        mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, k)

        # ── Conectar componentes: quedarse SOLO con el blob más grande ──
        # La cinta amarilla es continua → siempre es el blob más grande.
        # El ruido (piso beige, reflejos) son blobs pequeños y dispersos.
        # Esto es más robusto que un umbral de área fijo.
        n, lbls, stats, _ = cv2.connectedComponentsWithStats(mask, 8)

        if n <= 1:
            # No hay blobs → retornar vacío
            return np.zeros_like(mask)

        # Encontrar el blob más grande (ignorar fondo = label 0)
        areas = stats[1:, cv2.CC_STAT_AREA]   # areas sin el fondo
        largest = int(np.argmax(areas)) + 1    # +1 por el offset del fondo

        # Verificar que el blob más grande sea razonablemente grande
        # Si es muy pequeño (<0.05% del frame) → probablemente es ruido
        min_valid = max(50, int(w_img * h_img * 0.0005))
        if stats[largest, cv2.CC_STAT_AREA] < min_valid:
            return np.zeros_like(mask)

        # Quedarse solo con el blob más grande
        clean = np.zeros_like(mask)
        clean[lbls == largest] = 255

        # Dilatar un poco para rellenar huecos internos de la cinta
    ...
    def _draw(self, frame, binary, roi_mask, win_boxes,
        ...
        st   = 'OK' if self._age == 0 else \
               (f'MEM({self._age})' if poly is not None else '---')
        clr_st  = CLR_LINE if self._age == 0 else \
